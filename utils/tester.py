    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc="Testing"):
            ...
            if scaler is not None:
                ...
                pf_np = scaler.inverse_transform(pf_np.reshape(-1, pf_np.shape[-1])).reshape(orig_shape_f)
                tf_np = scaler.inverse_transform(tf_np.reshape(-1, tf_np.shape[-1])).reshape(orig_shape_f)
                tp_np = scaler.inverse_transform(tp_np.reshape(-1, tp_np.shape[-1])).reshape(orig_shape_p)
